...
#define GNOME_PROGRAM(obj)                                                     \
	(G_TYPE_CHECK_INSTANCE_CAST((obj), GNOME_TYPE_PROGRAM, GnomeProgram))
#define GNOME_PROGRAM_CLASS(klass)                                             \
	(G_TYPE_CHECK_CLASS_CAST((klass), GNOME_TYPE_PROGRAM, GnomeProgramClass))
#define GNOME_IS_PROGRAM(obj)                                                  \
	(G_TYPE_CHECK_INSTANCE_TYPE((obj), GNOME_TYPE_PROGRAM))
#define GNOME_IS_PROGRAM_CLASS(klass)                                          \
	(G_TYPE_CHECK_CLASS_TYPE((klass), GNOME_TYPE_PROGRAM))
...
GType gnome_program_get_type(void);
...
GType gnome_module_info_get_type(void);
 ...
 */
poptContext gnome_program_preinit(GnomeProgram *program, const char *app_id,
                                  const char *app_version, int argc,
                                  char **argv);
...
#define GNOME_PROGRAM_STANDARD_PROPERTIES                                      \
	GNOME_PARAM_APP_PREFIX, PREFIX, GNOME_PARAM_APP_SYSCONFDIR, SYSCONFDIR,      \
			GNOME_PARAM_APP_DATADIR, DATADIR, GNOME_PARAM_APP_LIBDIR, LIBDIR

GnomeProgram *gnome_program_init(const char *app_id, const char *app_version,
                                 const GnomeModuleInfo *module_info, int argc,
                                 char **argv, const char *first_property_name,
                                 ...);

GnomeProgram *gnome_program_initv(GType type, const char *app_id,
                                  const char *app_version,
                                  const GnomeModuleInfo *module_info, int argc,
                                  char **argv, const char *first_property_name,
                                  va_list args);
GnomeProgram *gnome_program_init_paramv(GType type, const char *app_id,
                                        const char *app_version,
                                        const GnomeModuleInfo *module_info,
                                        int argc, char **argv, guint nparams,
                                        GParameter *params);
