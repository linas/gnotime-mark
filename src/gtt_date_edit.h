...
#define GTT_DATE_EDIT(obj)                                                    \
  (G_TYPE_CHECK_INSTANCE_CAST ((obj), GTT_TYPE_DATE_EDIT, GttDateEdit))
#define GTT_DATE_EDIT_CLASS(klass)                                            \
  (G_TYPE_CHECK_CLASS_CAST ((klass), GTT_TYPE_DATE_EDIT, GttDateEditClass))
#define GTT_IS_DATE_EDIT(obj)                                                 \
  (G_TYPE_CHECK_INSTANCE_TYPE ((obj), GTT_TYPE_DATE_EDIT))
#define GTT_IS_DATE_EDIT_CLASS(klass)                                         \
  (G_TYPE_CHECK_CLASS_TYPE ((klass), GTT_TYPE_DATE_EDIT))
#define GTT_DATE_EDIT_GET_CLASS(obj)                                          \
  (G_TYPE_INSTANCE_GET_CLASS ((obj), GTT_TYPE_DATE_EDIT, GttDateEditClass))

typedef struct gtt_date_edit_s GttDateEdit;
typedef struct gtt_date_edit_private_s GttDateEditPrivate;
typedef struct gtt_date_edit_class_s GttDateEditClass;

struct gtt_date_edit_s
{
  GtkHBox hbox;
...
GtkWidget *gtt_date_edit_new (time_t the_time, gboolean show_time,
                              gboolean use_24_format);
GtkWidget *gtt_date_edit_new_flags (time_t the_time, GttDateEditFlags flags);

/* Note that everything that can be achieved with gtt_date_edit_new can
 * be achieved with gtt_date_edit_new_flags, so that's why this call
 * is like the _new_flags call */
void gtt_date_edit_construct (GttDateEdit *gde, time_t the_time,
...
void gtt_date_edit_set_popup_range (GttDateEdit *gde, int low_hour,
                                    int up_hour);
