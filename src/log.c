...
log_write (time_t t, const char *logstr)
{
	...
	GFileOutputStream *ostream
			= g_file_append_to (ofile, G_FILE_CREATE_PRIVATE, NULL, &error);
	if (NULL == ostream)
	{
	...
	struct log_data_s log_data[] = { { date, strlen (date) },
																	 { logstr, strlen (logstr) },
																	 { "\n", 1 },
																	 { NULL, 0 } };

	struct log_data_s *ptr       = &log_data[0];
	while (NULL != ptr->str)
	{
		gsize bytes_written;
		if (!g_output_stream_write_all (G_OUTPUT_STREAM (ostream), ptr->str,
																		ptr->len, &bytes_written, NULL, &error))
		{
			g_warning ("Failed to write to logfile \"%s\": %s", config_logfile_name,
								 error->message);
			g_error_free (error);
			error = NULL;
			break;
	...
	{
		g_warning ("Failed to close logfile \"%s\": %s", config_logfile_name,
							 error->message);
		g_error_free (error);
		error = NULL;
	}
