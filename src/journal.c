...
typedef struct wiggy_s
{
	...
	GFileOutputStream *ostream; // File output stream to write to the output file
...
file_write_helper (GttGhtml *pl, const char *str, size_t len, gpointer data)
{
	...
	while (TRUE)
	{
		...
		if (0 > bytes_written)
		{
			g_warning ("Failed to write to journal file: %s", error->message);
			g_error_free (error);
			error = NULL;
			break;
		}
...
save_to_gio (Wiggy *wig, const char *filename)
{
	...
	wig->ostream  = g_file_replace (wig->ofile, NULL, FALSE,
	...
	else
	{
		...
		{
			g_warning ("Unable to close the file \"%s\": %s", filename,
								 error->message);
			g_error_free (error);
			error = NULL;
		}
...
on_print_clicked_cb (GtkWidget *w, gpointer data)
{
	GtkWidget *msg = gtk_message_dialog_new (
			w, GTK_DIALOG_MODAL, GTK_MESSAGE_INFO, GTK_BUTTONS_OK,
