...
static void do_show_report(const char *, GttPlugin *, KvpFrame *, GttProject *,
                           gboolean, GList *);
...
wiggy_open(GttGhtml *pl, gpointer ud)
{
	...
	wig->html_stream = g_memory_output_stream_new(NULL, 0, g_realloc, g_free);
}
...
wiggy_close(GttGhtml *pl, gpointer ud)
{
	...
	g_object_unref(G_OBJECT(wig->html_stream));
}
...
wiggy_write(GttGhtml *pl, const char *str, size_t len, gpointer ud)
{
	...
	GError *err = NULL;
	if (len != g_output_stream_write(G_OUTPUT_STREAM(wig->html_stream), str, len,
	                                 NULL, &err) ||
	    err)
	{
		g_warning("Failed to write to HTML stream: %s\n", err->message);
		g_error_free(err);
		err = NULL;
	}
	webkit_web_view_load_string(
			wig->html,
			g_memory_output_stream_get_data(G_MEMORY_OUTPUT_STREAM(wig->html_stream)),
			NULL, NULL, "asdf");
}
...
{
	Wiggy *wig = (Wiggy *)ud;
	GMemoryOutputStream *mem_ostream = G_MEMORY_OUTPUT_STREAM(
	...
	if (404 == err)
	{
		...
		g_output_stream_write(G_OUTPUT_STREAM(mem_ostream), buff, p - buff, NULL,
		                      NULL);
		webkit_web_view_load_string(wig->html,
		                            g_memory_output_stream_get_data(
																		G_MEMORY_OUTPUT_STREAM(wig->html_stream)),
		                            NULL, NULL, "asdf");
	} else
	{
		p = buff;
	...
	g_object_unref(G_OBJECT(mem_ostream));
	g_object_unref(G_OBJECT(wig->html_stream));
}
...
static void
html_on_url_cb(WebKitWebView *web_view, const gchar *title, const gchar *uri,
               gpointer data)
{
	Wiggy *wig = data;
	if (NULL == wig->top)
               ...
               GttProject *prj, gboolean did_query, GList *prjlist)
{
	...
	wig->html = WEBKIT_WEB_VIEW(webkit_web_view_new());
	webkit_web_view_set_editable(wig->html, FALSE);
...
g_signal_connect(G_OBJECT(wig->html), "submit", G_CALLBACK(submit_clicked_cb),
	               wig);

g_signal_connect(G_OBJECT(wig->html), "url_requested",
	                 G_CALLBACK(html_url_requested_cb), wig); */

	g_signal_connect(G_OBJECT(wig->html), "hovering-over-link",
	...
	glade_xml_signal_connect_data(glxml, "on_merge_up_activate",
	                              GTK_SIGNAL_FUNC(interval_merge_up_clicked_cb),
	                              wig);

	glade_xml_signal_connect_data(glxml, "on_merge_down_activate",
	                              GTK_SIGNAL_FUNC(interval_merge_down_clicked_cb),
	                              wig);
	...
	glade_xml_signal_connect_data(glxml, "on_move_down_activate",
	                              GTK_SIGNAL_FUNC(interval_move_down_clicked_cb),
	                              wig);
	...
	glade_xml_signal_connect_data(glxml, "on_delete_times_activate",
	                              GTK_SIGNAL_FUNC(task_delete_times_clicked_cb),
	                              wig);
