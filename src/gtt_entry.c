...
#define DEFAULT_MAX_HISTORY_SAVED 10 /* This seems to make more sense then    \
                                        60*/
...
static void gnome_entry_get_property (GObject *object, guint param_id,
                                      GValue *value, GParamSpec *pspec);
static void gnome_entry_set_property (GObject *object, guint param_id,
  ...
  if (object_type == 0)
    {
      ...
      object_type = g_type_register_static (GTK_TYPE_COMBO, "GnomeEntry",
                                            &object_info, 0);

      g_type_add_interface_static (object_type, GTK_TYPE_EDITABLE,
                                   &editable_info);
    }
  return object_type;
}
...
static gboolean
gnome_entry_mnemonic_activate (GtkWidget *widget, gboolean group_cycling)
{
  gboolean handled;
  GnomeEntry *entry;
...
gnome_entry_class_init (GnomeEntryClass *class)
{
  ...
  gnome_entry_signals[ACTIVATE_SIGNAL] = g_signal_new (
      ...
      G_STRUCT_OFFSET (GnomeEntryClass, activate), NULL, NULL,
...
static void
gnome_entry_set_property (GObject *object, guint param_id, const GValue *value,
                          GParamSpec *pspec)
{
  GnomeEntry *entry = GNOME_ENTRY (object);
...
entry_activated (GtkWidget *widget, gpointer data)
{
  ...
  if (!gentry->_priv->changed || (strcmp (text, "") == 0))
    ...
    {
      gnome_entry_prepend_history (gentry, TRUE,
...
static void
gnome_entry_history_changed (GConfClient *client, guint cnxn_id,
...
gnome_entry_set_history_id (GnomeEntry *gentry, const gchar *history_id)
{
  ...
  gconf_client_add_dir (gentry->_priv->gconf_client, key,
                        GCONF_CLIENT_PRELOAD_NONE, NULL);

  gentry->_priv->gconf_notify_id = gconf_client_notify_add (
      gentry->_priv->gconf_client, key, gnome_entry_history_changed, gentry,
      NULL, NULL);
...
build_gconf_key (GnomeEntry *gentry)
{
  ...
  app_id
      = gconf_escape_key (gnome_program_get_app_id (gnome_program_get ()), -1);

  retval = g_strconcat ("/apps/gnome-settings/",
                        gnome_program_get_app_id (gnome_program_get ()),
                        "/history-", gentry->_priv->history_id, NULL);
...
static void
gnome_entry_add_history (GnomeEntry *gentry, gboolean save, const gchar *text,
                         gboolean append)
{
  GnomeEntryPrivate *priv;
  struct item *item;
...
gnome_entry_load_history (GnomeEntry *gentry)
{
  ...
  gconf_items = gconf_client_get_list (gentry->_priv->gconf_client, key,
...
static gboolean
check_for_duplicates (GSList *gconf_items, const struct item *item)
{
...
gnome_entry_save_history (GnomeEntry *gentry)
{
  ...
  gconf_client_set_list (gentry->_priv->gconf_client, key, GCONF_VALUE_STRING,
                         gconf_items, NULL);
