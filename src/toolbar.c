	...
	if (config_show_toolbar)
	{
		...
		if (config_show_tb_prop)
		{
			mytbar->prop_w = gtk_toolbar_insert_stock(
					...
					NULL, (GtkSignalFunc)menu_properties, NULL, position++);
		}
		if (config_show_tb_timer)
		{
			...
			gtk_image_set_from_stock(mytbar->timer_button_image, GNOME_STOCK_TIMER,
			                         GTK_ICON_SIZE_LARGE_TOOLBAR);
		...
		if (config_show_tb_calendar)
		{
			mytbar->calendar_w = toolbar_append_stock_button(
					...
					GNOME_STOCK_TEXT_BULLETED_LIST, (GtkSignalFunc)edit_calendar, NULL);
			position++;
		}
