...
struct export_format_s
{
  ...
  GFile *ofile;               // The file to export the data to
  GFileOutputStream *ostream; // The output stream to write to the export file
...
export_write (GttGhtml *gxp, const char *str, size_t len, export_format_t *xp)
{
  ...
  while (buflen > 0)
    {
      GError *error = NULL;
      const gsize bytes_written = g_output_stream_write (
          G_OUTPUT_STREAM (xp->ostream), &str[off], buflen, NULL, &error);
      if (0 > bytes_written)
        {
          g_warning ("Failed to write to export file \"%s\": %s", xp->uri,
                     error->message);
...
export_really (GtkWidget *widget, export_format_t *xp)
{
  ...
  GError *error = NULL;
  xp->ostream = g_file_replace (xp->ofile, NULL, FALSE, G_FILE_CREATE_NONE,
                                NULL, &error);
  if (NULL == xp->ostream)
    {
  ...
  if (FALSE
      == g_output_stream_close (G_OUTPUT_STREAM (xp->ostream), NULL, &error))
    {
