  ...
  if (G_UNLIKELY (dialog_type == 0))
    {
      ...
      dialog_type = g_type_register_static (GTK_TYPE_WINDOW, "GnomeDialog",
                                            &dialog_info, 0);
    }
...
gnome_dialog_init (GnomeDialog *dialog)
{
  ...
  gtk_box_pack_start (GTK_BOX (vbox), dialog->vbox, TRUE, TRUE,
                      GNOME_PAD_SMALL);
  ...
  if (button_name != NULL)
    {
      ...
      g_signal_connect_after (
          button, "clicked", G_CALLBACK (gnome_dialog_button_clicked), dialog);
...
gnome_dialog_run_real (GnomeDialog *dialog, gboolean close_after)
{
  ...
  ri.destroy_id = g_signal_connect (
      dialog, "destroy", G_CALLBACK (gnome_dialog_mark_destroy), &ri);
...
/**
 * gnome_dialog_set_close: Whether to call gnome_dialog_close() when a button
 *is clicked.
 * @dialog: #GnomeDialog to affect.
...
/**
 * gnome_dialog_editable_enters: Make the "activate" signal of an editable
