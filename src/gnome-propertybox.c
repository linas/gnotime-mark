...
gnome_property_box_class_init(GnomePropertyBoxClass *klass)
{
	...
	property_box_signals[APPLY] =
			g_signal_new("apply", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
	                 G_STRUCT_OFFSET(GnomePropertyBoxClass, apply), NULL, NULL,
	                 g_cclosure_marshal_VOID__INT, G_TYPE_NONE, 1, G_TYPE_INT);
	property_box_signals[HELP] =
			g_signal_new("help", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
...
gnome_property_box_instance_init(GnomePropertyBox *property_box)
{
	...
	gnome_dialog_append_buttons(GNOME_DIALOG(property_box),
	                            GNOME_STOCK_BUTTON_HELP, GNOME_STOCK_BUTTON_APPLY,
	                            GNOME_STOCK_BUTTON_CLOSE, GNOME_STOCK_BUTTON_OK,
	                            NULL);
...
{
        return NULL; /* TODO g_object_new(GNOME_TYPE_PROPERTY_BOX, NULL); */
}
...
dialog_clicked_cb(GnomeDialog *dialog, gint button, gpointer data)
{
	...
	/* TODO g_return_if_fail(GNOME_IS_PROPERTY_BOX(dialog)); */

	/* TODO pbox = GNOME_PROPERTY_BOX(dialog); */
	...
	if (cur_page_no >= 0)
	{
		...
		while (page != NULL)
		{
			...
			page = gtk_notebook_get_nth_page(GTK_NOTEBOOK(pbox->notebook), ++page_no);
		}
	} else
	{
		page = NULL;
	...
	{
	case 0:
		/* TODO help(GNOME_PROPERTY_BOX(dialog)); */
		break;
	case 1:
		/* TODO global_apply(GNOME_PROPERTY_BOX(dialog)); */
		break;
	case 2:
		/* TODO just_close(GNOME_PROPERTY_BOX(dialog)); */
		break;
	case 3:
		/* TODO if (dirty)
...
gnome_property_box_changed(GnomePropertyBox *property_box)
{
	...
	/* TODO g_return_if_fail(GNOME_IS_PROPERTY_BOX(property_box)); */
 ...
 */
gint
gnome_property_box_append_page(GnomePropertyBox *property_box, GtkWidget *child,
                               GtkWidget *tab_label)
{
	g_return_val_if_fail(property_box != NULL, -1);
	/* TODO g_return_val_if_fail(GNOME_IS_PROPERTY_BOX(property_box), -1); */
