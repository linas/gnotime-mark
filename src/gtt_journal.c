...
typedef struct wiggy_s
{
  ...
  GFileOutputStream *ostream; // Stream to write to the save file
  ...
  else
    {
      ...
      if (FALSE
          == g_output_stream_close (G_OUTPUT_STREAM (wig->ostream), NULL,
                                    &error))
        {
          g_warning ("Failed to close a file: %s", error->message);
                       ...
                       gpointer data)
{
  ...
  if (NULL == istream)
    {
      g_warning ("Failed to open file \"%s\" for reading: %s", path,
                 error->message);
...
#define BSZ 16000
  char buff[BSZ];
  gssize bytes_read = g_input_stream_read (G_INPUT_STREAM (istream), buff, BSZ,
                                           NULL, &error);
  while (0 < bytes_read)
    {
  ...
  if (FALSE == g_input_stream_close (G_INPUT_STREAM (istream), NULL, &error))
    {
      g_warning ("Failed to close file \"%s\" after reading: %s", path,
                 error->message);
