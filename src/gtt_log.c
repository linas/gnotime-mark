...
log_write (time_t t, const char *logstr)
{
  ...
  LogData log_data[] = { { date, strlen (date) },
                         { logstr, strlen (logstr) },
                         { "\n", 1 },
                         { NULL, 0 } };
  LogData *data_item = &log_data[0];
  while (NULL != data_item->data)
    {
      gsize bytes_written;
      if (FALSE
          == g_output_stream_write_all (G_OUTPUT_STREAM (log_ostream),
                                        data_item->data, data_item->len,
                                        &bytes_written, NULL, &error))
        {
          g_warning ("Failed to write to log file \"%s\": %s",
                     config_logfile_name, error->message);
  ...
  if (FALSE
    ...
    {
      g_warning ("Failed to close log file \"%s\": %s", config_logfile_name,
                 error->message);
