...
gtt_projects_tree_init(GttProjectsTree *gpt)
{
	...
	g_signal_connect(GTK_TREE_VIEW(gpt), "row-expanded",
	                 G_CALLBACK(gtt_projects_tree_row_expand_collapse_callback),
	                 NULL);
	g_signal_connect(GTK_TREE_VIEW(gpt), "row-collapsed",
	                 G_CALLBACK(gtt_projects_tree_row_expand_collapse_callback),
	                 NULL);
}
...
gtt_projects_tree_class_init(GttProjectsTreeClass *klass)
{
	...
	projects_tree_signals[COLUMNS_SETUP_DONE] = g_signal_new(
			"columns_setup_done", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
			0, NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}

static void
gtt_projects_tree_set_time_value(GttProjectsTree *gpt, GtkTreeStore *tree_model,
                                 GtkTreeIter *iter, gint column, gint value)
{
...
static void
gtt_projects_tree_set_date_value(GttProjectsTree *gpt, GtkTreeStore *tree_model,
                                 GtkTreeIter *iter, gint column, time_t value)
{
	gchar buff[100];
	memset(buff, 0, 100);
	if (value > -1)
                                    ...
                                    GtkTreeIter *iter)
{
	...
	{
		gtt_projects_tree_set_time_value(gpt, tree_model, iter, TIME_EVER_COLUMN,
		                                 gtt_project_get_secs_ever(prj));
		gtt_projects_tree_set_time_value(gpt, tree_model, iter, TIME_YEAR_COLUMN,
		                                 gtt_project_get_secs_year(prj));
		gtt_projects_tree_set_time_value(gpt, tree_model, iter, TIME_MONTH_COLUMN,
		                                 gtt_project_get_secs_month(prj));
		gtt_projects_tree_set_time_value(gpt, tree_model, iter, TIME_WEEK_COLUMN,
		...
		gtt_projects_tree_set_time_value(gpt, tree_model, iter, TIME_TODAY_COLUMN,
		                                 gtt_project_get_secs_day(prj));
		gtt_projects_tree_set_time_value(gpt, tree_model, iter, TIME_TASK_COLUMN,
                                    ...
                                    GtkTreeIter *iter)
{
	...
	gtt_projects_tree_set_date_value(gpt, tree_model, iter, ESTIMATED_END_COLUMN,
                                   ...
                                   GtkTreeIter *iter)
{
	...
	gtk_tree_store_set(tree_model, iter, TITLE_COLUMN, gtt_project_get_title(prj),
	                   DESCRIPTION_COLUMN, gtt_project_get_desc(prj), TASK_COLUMN,
	                   gtt_task_get_memo(gtt_project_get_current_task(prj)),
	                   SIZING_COLUMN, gtt_project_get_sizing(prj), PERCENT_COLUMN,
	                   gtt_project_get_percent_complete(prj), GTT_PROJECT_COLUMN,
	                   prj, -1);
                              ...
                              GtkTreePath *path, gboolean recursive)
{
	...
	if (recursive)
	{
		     ...
		     node = node->next, gtk_tree_path_next(child_path))
		{
			...
			gtt_projects_tree_add_project(gpt, tree_model, sub_prj, &iter, child_path,
			                              recursive);
		}
...
gtt_projects_tree_populate_tree_store(GttProjectsTree *gpt,
                                      GtkTreeStore *tree_model, GList *prj_list,
                                      gboolean recursive)
{
	GList *node;
...
gtt_projects_tree_add_column(GttProjectsTree *project_tree, gchar *column_name)
{
	...
	for (index = 0; index < N_VIEWABLE_COLS; index++)
	{
		if (!strcmp(column_name, priv->column_definitions[index].name))
		{
			...
			{
				gtk_tree_view_column_add_attribute(column, c->renderer, "weight",
				                                   WEIGHT_COLUMN);
			}
                                              ...
                                              GttProject *parent)
{
	...
	{
		gtt_projects_tree_append_projects_recursively(gpt, model, row_references,
		                                              children->data, prj);
	}
}
                                    ...
                                    GtkTreeViewColumn *column)
{
	...
	for (; p != NULL; p = p->next)
	{
		if (p->data == column)
		{
			gtk_tree_view_column_set_sort_indicator(GTK_TREE_VIEW_COLUMN(p->data),
			                                        TRUE);
