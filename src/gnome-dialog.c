...
void
_gnome_marshal_BOOLEAN__VOID(GClosure *closure, GValue *return_value,
...
gnome_dialog_get_type(void)
{
	...
	if (G_UNLIKELY(dialog_type == 0))
	{
		...
		dialog_type =
				g_type_register_static(GTK_TYPE_WINDOW, "GnomeDialog", &dialog_info, 0);
	}
...
gnome_dialog_class_init(GnomeDialogClass *klass)
{
	...
	dialog_signals[CLOSE] =
			g_signal_new("close", G_TYPE_FROM_CLASS(gobject_class), G_SIGNAL_RUN_LAST,
	                 G_STRUCT_OFFSET(GnomeDialogClass, close), NULL, NULL,
	...
	dialog_signals[CLICKED] = g_signal_new(
			...
			G_STRUCT_OFFSET(GnomeDialogClass, clicked), NULL, NULL,
...
gnome_dialog_init(GnomeDialog *dialog)
{
	...
	gtk_box_pack_start(GTK_BOX(vbox), dialog->vbox, TRUE, TRUE, GNOME_PAD_SMALL);
...
/**
 * gnome_dialog_construct: Functionality of gnome_dialog_new() for language
 *wrappers.
...
/**
 * gnome_dialog_constructv: Functionality of gnome_dialog_new(), for language
 *wrappers.
 * @dialog: Dialog to construct.
 * @title: Title of the dialog.
 ...
 **/
void
gnome_dialog_constructv(GnomeDialog *dialog, const gchar *title,
                        const gchar **buttons)
{
 ...
 * @...: NULL-terminated varargs list of button names or GNOME_STOCK_BUTTON_*
 *defines.
 *
...
/**
 * gnome_dialog_append_buttons: Add buttons to a dialog after its initial
 *construction.
 ...
 **/
void
gnome_dialog_append_button(GnomeDialog *dialog, const gchar *button_name)
{
	g_return_if_fail(dialog != NULL);
	g_return_if_fail(GNOME_IS_DIALOG(dialog));
...
gnome_pixmap_button(GtkWidget *pixmap, const char *text)
{
	...
	hbox = gtk_hbox_new(FALSE, 0);
	gtk_widget_show(hbox);
	gtk_box_pack_start(GTK_BOX(w), hbox, TRUE, FALSE, GNOME_STOCK_BUTTON_PADDING);
 ...
 **/
void
gnome_dialog_append_button_with_pixmap(GnomeDialog *dialog,
                                       const gchar *button_name,
...
/**
 * gnome_dialog_append_buttonsv: Like gnome_dialog_append_buttons(), but with a
...
/**
 * gnome_dialog_append_buttons_with_pixmaps: Like
 *gnome_dialog_append_button_with_pixmap(), but allows multiple buttons.
 * @dialog: #GnomeDialog to append to.
 ...
 **/
void
gnome_dialog_append_buttons_with_pixmaps(GnomeDialog *dialog,
                                         const gchar **names,
                                         const gchar **pixmaps)
{
	g_return_if_fail(dialog != NULL);
...
static void
gnome_dialog_setbutton_callback(GnomeDialog *dialog, gint button_number,
                                struct GnomeDialogRunInfo *runinfo)
{
...
static gboolean
gnome_dialog_quit_run(GnomeDialog *dialog, struct GnomeDialogRunInfo *runinfo)
{
	if (runinfo->close_id < 0)
		return FALSE;
...
gnome_dialog_run_real(GnomeDialog *dialog, gboolean close_after)
{
	...
	ri.clicked_id = g_signal_connect(
			dialog, "clicked", G_CALLBACK(gnome_dialog_setbutton_callback), &ri);

	ri.close_id =
			g_signal_connect(dialog, "close", G_CALLBACK(gnome_dialog_quit_run), &ri);
...
/**
 * gnome_dialog_run_and_close: Like gnome_dialog_run(), but force-closes the
...
/**
 * gnome_dialog_set_default: Set the default button for the dialog. The Enter
...
/**
 * gnome_dialog_set_close: Whether to call gnome_dialog_close() when a button is
 *clicked.
 * @dialog: #GnomeDialog to affect.
...
/**
 * gnome_dialog_close_hides: gnome_dialog_close() can destroy or hide the
 *dialog; toggle this behavior.
 * @dialog: #GnomeDialog to affect.
 * @just_hide: If TRUE, gnome_dialog_close() calls gtk_widget_hide() instead of
 *gtk_widget_destroy().
 *
...
/**
 * gnome_dialog_button_connect: Connect a callback to one of the button's
 *"clicked" signals.
 * @dialog: #GnomeDialog to affect.
 ...
 **/
void
gnome_dialog_button_connect(GnomeDialog *dialog, gint button,
 ...
 **/
void
gnome_dialog_button_connect_object(GnomeDialog *dialog, gint button,
 ...
 **/
void
gnome_dialog_set_accelerator(GnomeDialog *dialog, gint button,
...
/**
 * gnome_dialog_editable_enters: Make the "activate" signal of an editable click
...
static void
gnome_dialog_button_clicked(GtkWidget *button, GtkWidget *dialog)
{
	GList *list;
	int which = 0;
...
static gint
gnome_dialog_key_pressed(GtkWidget *d, GdkEventKey *e)
{
...
static gint
gnome_dialog_delete_event(GtkWidget *d, GdkEventAny *e)
{
	gnome_dialog_close(GNOME_DIALOG(d));
...
static void
gnome_dialog_destroy(GtkObject *object)
{
	GnomeDialog *dialog;
...
static void
gnome_dialog_finalize(GObject *object)
{
	g_return_if_fail(object != NULL);
	g_return_if_fail(GNOME_IS_DIALOG(object));
...
static void
gnome_dialog_close_real(GnomeDialog *dialog)
{
	g_return_if_fail(dialog != NULL);
	g_return_if_fail(GNOME_IS_DIALOG(dialog));
