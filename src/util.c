...
gtt_builder_xml_new(const char *filename, const char *widget)
{
	...
	{
		builder = gtk_builder_new(); // filename, widget, NULL);
	...
	if (builder == NULL)
	{
		...
		builder = gtk_builder_new(); // file, widget, NULL);
...
xxxqof_print_date_time_buff(char *buff, size_t len, time_t secs)
{
	...
	case QOF_DATE_FORMAT_ISO:
		flen = g_snprintf(buff, len, "%04d-%02d-%02d %02d:%02d", year, month, day,
		                  hour, min);
		break;
...
size_t
xxxqof_print_date_dmy_buff(char *buff, size_t len, int day, int month, int year)
{
	int flen;
	if (!buff)
		return 0;
