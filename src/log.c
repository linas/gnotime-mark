...
log_write(time_t t, const char *logstr)
{
	...
	gsize bytes_written;
	g_output_stream_write_all(G_OUTPUT_STREAM(ostream), date, strlen(date),
	                          &bytes_written, NULL, &error);
	g_output_stream_write_all(G_OUTPUT_STREAM(ostream), logstr, strlen(logstr),
	                          &bytes_written, NULL, &error);
	g_output_stream_write_all(G_OUTPUT_STREAM(ostream), "\n", 1, &bytes_written,
	                          NULL, &error);

	if (!g_output_stream_close(G_OUTPUT_STREAM(ostream), NULL, &error) || error)
	{
		// TODO: Report error
		g_clear_error(&error);
	}
...
printf_project(const char *format, GttProject *proj)
{
	...
	for (p = format; *p; p++)
	{
		...
		} else
		{
			...
			case 'T':
				sss = gtt_project_get_secs_ever(proj);
				g_string_append_printf(str, "%d:%02d:%02d", sss / 3600, (sss / 60) % 60,
				                       sss % 60);
				break;
			case 'r': {
