...
struct export_format_s
{
	...
	GFile   *file_obj;               /* file object */
	GFileOutputStream *file_ostream; /* file output stream */
			  ...
			  export_format_t *xp)
{
	...
	while ((buflen > 0) && (bytes_written >= 0) && !err)
	{
		bytes_written = g_output_stream_write(G_OUTPUT_STREAM(xp->file_ostream), &str[off], buflen, NULL, &err);
		off += bytes_written;
...
export_really (GtkWidget *widget, export_format_t *xp)
{
	...
	GError * err = NULL;
	xp->file_obj = g_file_new_for_path(xp->uri);
	xp->file_ostream = g_file_append_to(xp->file_obj, G_FILE_CREATE_NONE, NULL, &err);
	if (!xp->file_ostream || err)
	{
...
cleanup:
	if (!g_output_stream_close(G_OUTPUT_STREAM(xp->file_ostream), NULL, &err) || err) {
		/* TODO: Report error but don't return */
	}
	xp->file_ostream = NULL;
	g_object_unref(xp->file_obj);
	xp->file_obj = NULL;
}
